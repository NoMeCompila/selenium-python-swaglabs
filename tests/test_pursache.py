...
def test_buy_products(init_driver: WebDriver) -> None:
    ...
    assert overview_page.get_text(overview_page.title) == "CHECKOUT: OVERVIEW"
    assert len(overview_page.list_all_elements(overview_page.items_price)) == 2
    #print(overview_page.get_items_cost(overview_page.items_price))
    assert overview_page.get_items_cost(overview_page.items_price) == 65.98
    print(overview_page.get_text(overview_page.tax_price))

    assert overview_page.get_total_cost(overview_page.items_price, overview_page.tax_price) == overview_page.clear_total(overview_page.sumary_total)
